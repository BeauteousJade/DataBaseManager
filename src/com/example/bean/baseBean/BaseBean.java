...
/**
 * 本类是用来供其他Bean继承的，通过继承本类的Bean类，可以保存在数据库中的一张表，一个类对应一张表
 * 
 * @author apple
 *
 */
public abstract class BaseBean {
	 ...
	 * 如果定义了的话，会出现错误的。同时自定义的属性不能使用@MyAnnotation(fieldMark = MyAnnotation.FieldMark.ID)
	 * 注解，如果定义了本注解的话，对应的数据库表会出现两个主键。
	 * 
	...
	public String[] getFieldValues() {
		...
		for (int i = 0; i < fieldValues.length; i++) {
			//将一个属性名格式话需要的格式，例如，一个属性名：userName，通过本步骤，变为UserName
	...
	/**
	 * 本方法获取一个类中非空的属性名的值，包括父类的所有属性，private，protected，default，public都在内
	 * @return 返回获取的属性值数组
	 */
	 ...
	 * @param fields 该类对象对象需要设置的属性名数组
	 * @param values 该类对象需要设置的属性值数组
